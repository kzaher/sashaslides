#!/usr/bin/env node
// genspec-vm.mjs <config.json> <out.json> [--shim] [--persist usr/local,root,home,var]
...
//              bundle/persist/<p> over the built-in virtio-9p root device, the guest init mounts that
//              device at /mnt/wasi0, and runc bind-mounts /mnt/wasi0/bundle/persist/<p> onto /<p> in the
//              container — writes go through the 9p share into the host tree (journaled to OPFS).
...
if (persist.length) {
  spec.mounts = (spec.mounts || []).filter((m) => !(m.source || "").startsWith("/mnt/wasi0/bundle/persist/"));
...
console.log(`spec: ${outFile} (${shim ? "shim: PATH=" + spec.process.env.find((k) => k.startsWith("PATH=")) + ", /dev/hvc1; " : ""}persist: ${persist.join(",") || "none"})`);
