...
`web/sandbox.html?cli=claude|codex|agy` — the user's direction for the system-node track: **ship only a
...
1. **Three things start at once**: the VM worker (`sandbox-worker.js`) fetches the slim engine
   ...
   yet; for `cli=claude` the runtime worker (`native/runtime.js`) starts and waits for the guest shim.
...
7. **The CLI starts** (`sh -c 'printf "\e]777;nb-booted\a"; exec …'` — an OSC xterm swallows and the page
   timestamps as "guest booted"): claude = `/usr/local/bin/claude` → the shim → the runtime worker reads
...
* Package versions are pinned in `NanoboxInstaller.CATALOG` (2.1.112 / 0.147.0 / node 22.23.2 /
  npm 10.9.8); the agy manifest is cached with the package (`?reset=1` re-resolves).
