...
| runtime-side client (syscall backend #2) | `web/native/guest.js` `NanoboxGuest.connect({port, ringSab})` → async `open/read/stat/spawn/…`, events `onHello/onStdin/onChildOut/…` | done |
| guest shim `nbnode` (installed as `/usr/local/bin/node`) | `guest/nbnode/nbnode.c` (static musl, x86-64): HELLO with argv/env/cwd/tty, poll loop serving requests with real syscalls, fork/exec for SPAWN, tty raw mode, signals | in progress (agent) |
