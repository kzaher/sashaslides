...
// nanobox/claude fork of container2wasm's worker-util.js startContainer():
//   opts.bundle    the --external-bundle argument (default: the network 9p server, as upstream)
//   opts.extraFds  additional WASI fd objects appended after the socket fds (fd 6, 7, ...)
//   opts.args      extra runtime arguments (before the container command)
//   opts.beforeStart(inst, wasi)  called once the engine is instantiated, before wasi.start()
//   opts.wasm      already-fetched (decompressed) module bytes; when absent info.vmImage is fetched
export function startContainer(info, cargs, ttyClient, opts) {
    opts = opts || {};
    ...
    const go = (wasm) => {
        ...
        if (opts.extraFds) fds = fds.concat(opts.extraFds);
        args = ['arg0', '--net=socket=listenfd=4', '--mac', genmac(), '--external-bundle=' + (opts.bundle || '9p=192.168.127.252')];
        ...
        listenfd = 4;
        startWasi(wasm, ttyClient, args, env, fds, listenfd, 5, opts);
    };
    if (opts.wasm) { go(opts.wasm); return; }
    fetch(info.vmImage, { credentials: 'same-origin' }).then((resp) => {
        resp['blob']().then((blob) => {
            const ds = new DecompressionStream("gzip")
            new Response(blob.stream().pipeThrough(ds))['arrayBuffer']().then(go);
        })
    });
}

function startWasi(wasm, ttyClient, args, env, fds, listenfd, connfd, opts) {
    ...
    if (opts && opts.beforeInstantiate) opts.beforeInstantiate(wasi);
    WebAssembly.instantiate(wasm, {
        "wasi_snapshot_preview1": wasi.wasiImport,
    }).then((inst) => {
        if (opts && opts.beforeStart) opts.beforeStart(inst.instance, wasi);
        wasi.start(inst.instance);
    });
}
export { WASI, PreopenDirectory, File, wasitype };
