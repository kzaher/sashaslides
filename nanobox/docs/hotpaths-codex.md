...
8. Ranked levers (§5): (1) keep hotness across evictions + bigger/associative iCache + no full flush
   ...
   full TLB probe, multi-entry probe cache kept across blocks) so the threshold can drop to ~200 with 85 % coverage
...
**1.8 KB of wasm**, 8 instructions 3.0 KB, 20 instructions 5.9 KB — every memory operand inlines a full DTLB probe
(~45 ops) plus the single-entry probe cache and a slow-path call site. Lukewarm traces of ~7 instructions execute
...
| 3 | **Make short JIT'd traces cheaper than the interpreter**, then lower T to ~200–500 (85 % coverage). (a) Successor cache in the link epilogue: store `(expected pAddr, jitfn, jitarg, ilen)` of the last successor in the trace/region descriptor and verify only `pAddr` — replaces the fetchModeMask/iCache-index/entry loads (6 loads + mul) by 2 loads; fall back to today's lookup on mismatch. (b) Move the full DTLB probe + probe-cache refill (`tlbProbeFull` + refill, ~35 ops) into ONE helper function per batch module (`call`, same instance) and inline only the probe-cache hit path; expected code size 200 → ~80 B per guest instruction (V8 machine code ~3×), which is what makes lukewarm code i-cache-resident. (c) 2–4-entry probe cache keyed by class (RSP-based vs other base register) kept in locals across in-region transitions like registers (A3) — today the cache is one entry and is reset at every trace/block entry (`local 42 = -1`). (d) Trim the prologue: `user_pl`/`async_event` loads only when the trace has a memory op / needs the check. Success criterion: the T=200 run must get *faster* than T=2000, not 2.2 s slower. | **−1.0…1.5 s** (JIT share 60 % → 85 % at ≤ 5 ns/instr; interpreter handlers 2.2 s → ~0.6 s; cpu_loop 1.17 → ~0.5 s) | medium (new code paths in the epilogue/probe; the bisector + identity gate exist) | M–L |
