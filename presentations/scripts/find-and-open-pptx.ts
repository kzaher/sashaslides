/**
 * Find the uploaded presentation.pptx in Drive, get its file ID,
  ...
  const fileResult = await Runtime.evaluate({
    expression: `(() => {
      ...
      for (const item of items) {
        ...
        if (text.includes('presentation') || text.includes('.pptx')) {
      ...
      for (const t of tooltips) {
        ...
        if (tt.includes('presentation') || tt.includes('.pptx')) {
