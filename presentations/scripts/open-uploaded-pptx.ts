...
async function main() {
  ...
  // Find the presentation.pptx file in Drive
  const fileResult = await Runtime.evaluate({
    expression: `(() => {
      ...
      const tooltips = document.querySelectorAll('[data-tooltip*="presentation"]');
  ...
  if (!fileResult.result.value) {
    console.log("Could not find presentation.pptx in Drive view. Searching all files...");
