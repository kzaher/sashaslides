 ...
 * the whiteboard-styled slides from presentation into the original
 * presentation. This is the proper way to migrate slides with styles.
 *
 * Steps:
 * 1. File menu > Import slides
 * 2. Picker dialog opens: navigate to presentation
...
async function main() {
  ...
  console.log("In picker dialog, looking for presentation...");
  ...
  await Input.insertText({ text: "presentation" });
  await sleep(2500);

  // Look for the file in the results (in main DOM — picker contents sometimes reflected there)
  console.log("Looking for presentation in results...");
  const fileResult2 = await Runtime.evaluate({
    expression: `(() => {
      ...
      for (const el of all) {
        ...
        if (text === 'presentation' || text === 'presentation.pptx' || text.startsWith('presentation')) {
