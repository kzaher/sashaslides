 ...
 * 3. Select the presentation (styled) file
...
async function main() {
  ...
  // Step 3: In the picker dialog, find and click the presentation file
  console.log("Step 3: Finding presentation in picker...");
  ...
  // Try to click the presentation file that appears
  // The picker results appear inside an iframe — access via main page DOM
  const clickResult = await Runtime.evaluate({
    expression: `(() => {
      // Search for any visible element containing "presentation"
      const all = document.querySelectorAll('*');
      for (const el of all) {
        if (el.children.length === 0) {
          const text = (el.textContent || '').trim();
          if (text === 'presentation' || text === 'presentation.pptx') {
  ...
  if (clickResult.result.value) {
    ...
    console.log(`  Clicking presentation at ${pos.x}, ${pos.y}`);
    await clickAt(Input, pos.x, pos.y);
    await sleep(1000);
  } else {
    console.log("  Could not find presentation in picker view via main DOM");
  }
