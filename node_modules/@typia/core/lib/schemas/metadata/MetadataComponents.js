...
class MetadataComponents {
    ...
    static from(json) {
        ...
        for (const alias of json.aliases)
            (0, Writable_1.Writable)(dictionary.aliases.get(alias.name)).value = MetadataSchema_1.MetadataSchema.from(alias.value, dictionary);
        for (const array of json.arrays)
            (0, Writable_1.Writable)(dictionary.arrays.get(array.name)).value = MetadataSchema_1.MetadataSchema.from(array.value, dictionary);
        for (const tuple of json.tuples)
            (0, Writable_1.Writable)(dictionary.tuples.get(tuple.name)).elements =
                tuple.elements.map((elem) => MetadataSchema_1.MetadataSchema.from(elem, dictionary));
        // FINALIZE
