...
class MetadataSchema {
    ...
    static initialize(parentResolved = false) {
        const meta = MetadataSchema.create({
            any: false,
            nullable: false,
            required: true,
            optional: false,
...
(function (MetadataSchema) {
    MetadataSchema.intersects = (x, y) => {
        ...
        if (x.escaped && y.escaped)
            return (MetadataSchema.intersects(x.escaped.original, y.escaped.original) ||
                MetadataSchema.intersects(x.escaped.returns, y.escaped.returns));
        //----
        // VALUES
        //----
        // ATOMICS
    ...
    MetadataSchema.covers = (x, y, level = 0, escaped = false) => {
        // CHECK ANY
        if (x === y)
            return false;
        else if (x.any)
        ...
        if (escaped === false) {
            if (x.escaped === null && y.escaped !== null)
                ...
                y.escaped !== null &&
                (!MetadataSchema.covers(x.escaped.original, y.escaped.original, level, true) ||
                    !MetadataSchema.covers(x.escaped.returns, y.escaped.returns, level, true)))
                return false;
        }
        //----
        // INSTANCES
        ...
        if (level === 0) {
            ...
            for (const ya of y.arrays)
                if (!x.arrays.some((xa) => MetadataSchema.covers(xa.type.value, ya.type.value, level + 1))) {
                    return false;
                }
            // TUPLES
                ...
                if (yt.type.elements.length !== 0 &&
                    x.tuples.some((xt) => xt.type.elements.length >= yt.type.elements.length &&
                        xt.type.elements
                            ...
                            .every((xv, i) => MetadataSchema.covers(xv, yt.type.elements[i], level + 1))) === false)
                    return false;
        }
        // OBJECTS
        ...
        for (const ys of y.sets)
            if (x.sets.some((xs) => MetadataSchema.covers(xs.value, ys.value)) === false)
                return false;
        //----
        // VALUES
        //----
    ...
    /** @internal */
    MetadataSchema.merge = (x, y) => {
        var _a, _b;
        const output = MetadataSchema.create({
            any: x.any || y.any,
            ...
            escaped: x.escaped !== null && y.escaped !== null
                ? MetadataEscaped_1.MetadataEscaped.create({
                    original: MetadataSchema.merge(x.escaped.original, y.escaped.original),
                    returns: MetadataSchema.merge(x.escaped.returns, y.escaped.returns),
                })
            ...
            rest: x.rest !== null && y.rest !== null
                ? MetadataSchema.merge(x.rest, y.rest)
        ...
        for (const constant of y.constants) {
            const target = utils_1.ArrayUtil.take(output.constants, (elem) => elem.type === constant.type, () => MetadataConstant_1.MetadataConstant.create({
                type: constant.type,
                values: [],
            }));
            for (const value of constant.values)
                utils_1.ArrayUtil.add(target.values, value, (a, b) => a.value === b.value);
        }
        return output;
    };
    /** @internal */
    MetadataSchema.unalias = (w) => {
