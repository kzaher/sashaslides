...
import { MetadataSchema } from './MetadataSchema.mjs';
...
class MetadataComponents {
    ...
    static from(json) {
        ...
        for (const alias of json.aliases)
            Writable(dictionary.aliases.get(alias.name)).value = MetadataSchema.from(alias.value, dictionary);
        for (const array of json.arrays)
            Writable(dictionary.arrays.get(array.name)).value = MetadataSchema.from(array.value, dictionary);
