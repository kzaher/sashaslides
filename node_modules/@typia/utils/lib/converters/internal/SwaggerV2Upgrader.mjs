...
import { SwaggerV2TypeChecker } from '../../validators/SwaggerV2TypeChecker.mjs';
...
(function (SwaggerV2Upgrader) {
    ...
    const convertOperation = (doc) => (pathItem) => (input) => ({
        ...
        parameters: pathItem.parameters !== undefined || input.parameters !== undefined
            ? [...(pathItem.parameters ?? []), ...(input.parameters ?? [])]
                .map((p) => SwaggerV2TypeChecker.isReference(p)
                ? doc.parameters?.[p.$ref.split("/").pop() ?? ""]
                : p)
                .filter((p) => p !== undefined &&
                p.in !== "body" &&
                p.schema ===
                    undefined).map(convertParameter(doc.definitions ?? {}))
            : undefined,
        requestBody: (() => {
    ...
    SwaggerV2Upgrader.convertComponents = (input) => ({
        schemas: Object.fromEntries(Object.entries(input.definitions ?? {})
            .filter(([_, v]) => v !== undefined)
            .map(([key, value]) => [
            key,
            SwaggerV2Upgrader.convertSchema(input.definitions ?? {})(value),
        ])),
    ...
    SwaggerV2Upgrader.convertSchema = (definitions) => (input) => {
        ...
        const visit = (schema) => {
            ...
            // ATOMIC TYPE CASE (CONSIDER ENUM VALUES)
            else if (SwaggerV2TypeChecker.isBoolean(schema) ||
                ...
                else if (SwaggerV2TypeChecker.isInteger(schema) ||
                    SwaggerV2TypeChecker.isNumber(schema))
    ...
    const retrieveObject = (definitions) => (input, visited = new Set()) => {
        if (SwaggerV2TypeChecker.isObject(input))
            ...
            visited.add(input);
        if (SwaggerV2TypeChecker.isReference(input))
            return retrieveObject(definitions)(definitions?.[input.$ref.split("/").pop() ?? ""] ?? {}, visited);
        return null;
    };
