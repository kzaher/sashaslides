import '../../utils/ArrayUtil.mjs';
import '../../utils/LlmJson.mjs';
import { MapUtil } from '../../utils/MapUtil.mjs';
import '../../utils/NamingConvention.mjs';
import '../../utils/StringUtil.mjs';
...
(function (OpenApiOneOfValidator) {
    ...
    const getDiscriminator = (ctx) => {
        ...
        const branches = [
            ...(tuples.length === 0 && arrays.length !== 0
                ? discriminateArrays(ctx, significant.filter((flat) => OpenApiTypeChecker.isArray(flat.schema)))
                : []),
            ...discriminateObjects(ctx, significant.filter((flat) => OpenApiTypeChecker.isObject(flat.escaped)), tuples.length + arrays.length === 0),
        ];
        return {
            branches,
    ...
    const discriminateObjects = (ctx, objectSchemas, noArray) => {
        ...
        objectSchemas = objectSchemas
            ...
            escaped: {
                ...flat.escaped,
                properties: Object.fromEntries(Object.entries(flat.escaped.properties ?? {}).map(([key, value]) => [
                    key,
                    getFlattened({
                        components: ctx.components,
                        schema: value,
        ...
        objectSchemas.forEach((obj, i) => {
            for (const [key, value] of Object.entries(obj.escaped.properties ?? {})) {
                if (!!obj.escaped.required?.includes(key) === false)
                    continue;
        ...
        return objectSchemas
            ...
            for (const [key, value] of Object.entries(obj.escaped.properties ?? {})) {
                ...
                const unique = OpenApiTypeChecker.isConstant(value)
                    ? neighbors.every((oppo) => OpenApiTypeChecker.isConstant(oppo) &&
