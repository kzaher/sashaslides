...
import { EndpointUtil } from '../../utils/internal/EndpointUtil.mjs';
...
(function (HttpMigrateRouteComposer) {
    HttpMigrateRouteComposer.compose = (props) => {
        ...
        const body = emplaceBodySchema("request")((schema) => emplaceReference({
            ...
            name: EndpointUtil.pascal(`I/Api/${props.path}`) +
                "." +
        ...
        const [headers, query] = ["header", "query"].map((type) => {
            ...
            const objects = parameters
                .map((p) => OpenApiTypeChecker.isObject(p.schema)
                ? p.schema
                : OpenApiTypeChecker.isReference(p.schema) &&
                    OpenApiTypeChecker.isObject(props.document.components.schemas?.[p.schema.$ref.replace(`#/components/schemas/`, ``)] ?? {})
                    ? p.schema
                    : null)
                .filter((s) => !!s);
            const primitives = parameters.filter((p) => OpenApiTypeChecker.isBoolean(p.schema) ||
            ...
            const dto = objects[0]
                ? OpenApiTypeChecker.isObject(objects[0])
                    ? objects[0]
                    : (props.document.components.schemas ?? {})[objects[0].$ref.replace(`#/components/schemas/`, ``)]
                : null;
            const entire = [
                ...objects.map((o) => OpenApiTypeChecker.isObject(o)
                    ? o
                    : props.document.components.schemas?.[o.$ref.replace(`#/components/schemas/`, ``)]),
                {
                    type: "object",
        ...
        // PATH PARAMETERS
        //----
        const parameterNames = EndpointUtil.splitWithNormalization(props.emendedPath)
            .filter((str) => str[0] === ":")
        ...
        const parameters = (props.operation.parameters ?? [])
            ...
            key: (() => {
                ...
                if (NamingConvention.variable(key))
                    return key;
                while (true) {
                    key = "_" + key;
        ...
        return {
            ...
            exceptions: Object.fromEntries(Object.entries(props.operation.responses ?? {})
    ...
    const isNotObjectLiteral = (schema) => OpenApiTypeChecker.isReference(schema) ||
