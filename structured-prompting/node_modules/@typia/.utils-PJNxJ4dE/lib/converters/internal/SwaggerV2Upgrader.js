...
const SwaggerV2TypeChecker_1 = require("../../validators/SwaggerV2TypeChecker");
...
(function (SwaggerV2Upgrader) {
    SwaggerV2Upgrader.convert = (input) => ({
        openapi: "3.2.0",
        info: input.info,
        components: SwaggerV2Upgrader.convertComponents(input),
        paths: input.paths
    ...
    const convertOperation = (doc) => (pathItem) => (input) => {
        ...
        return (Object.assign(Object.assign({}, input), { parameters: pathItem.parameters !== undefined || input.parameters !== undefined
                ? [...((_a = pathItem.parameters) !== null && _a !== void 0 ? _a : []), ...((_b = input.parameters) !== null && _b !== void 0 ? _b : [])]
                    .map((p) => {
                    var _a, _b;
                    return SwaggerV2TypeChecker_1.SwaggerV2TypeChecker.isReference(p)
                ...
                const found = (_a = input.parameters) === null || _a === void 0 ? void 0 : _a.find((p) => {
                    var _a, _b;
                    if (SwaggerV2TypeChecker_1.SwaggerV2TypeChecker.isReference(p))
    ...
    const convertResponse = (doc) => (input) => {
        var _a, _b, _c;
        if (SwaggerV2TypeChecker_1.SwaggerV2TypeChecker.isReference(input)) {
    ...
    /* -----------------------------------------------------------
      DEFINITIONS
    ----------------------------------------------------------- */
    SwaggerV2Upgrader.convertComponents = (input) => {
        var _a;
        return ({
    ...
    SwaggerV2Upgrader.convertSchema = (definitions) => (input) => {
        ...
        const union = [];
        const attribute = Object.assign({ title: input.title, description: input.description, deprecated: input.deprecated, readOnly: input.readOnly, example: input.example, examples: input.examples
                ? Object.fromEntries(input.examples.map((v, i) => [`v${i}`, v]))
                : undefined }, Object.fromEntries(Object.entries(input).filter(([key, value]) => key.startsWith("x-") && value !== undefined)));
        const visit = (schema) => {
            ...
            if (Array.isArray(schema.enum) &&
                ...
                nullable.value || (nullable.value = true);
            // UNION TYPE CASE
            if (SwaggerV2TypeChecker_1.SwaggerV2TypeChecker.isAnyOf(schema))
                schema["x-anyOf"].forEach(visit);
            else if (SwaggerV2TypeChecker_1.SwaggerV2TypeChecker.isOneOf(schema))
                schema["x-oneOf"].forEach(visit);
            else if (SwaggerV2TypeChecker_1.SwaggerV2TypeChecker.isAllOf(schema))
                if (schema.allOf.length === 1)
                    visit(schema.allOf[0]);
                else
                    union.push(convertAllOfSchema(definitions)(schema));
            // ATOMIC TYPE CASE (CONSIDER ENUM VALUES)
            else if (SwaggerV2TypeChecker_1.SwaggerV2TypeChecker.isBoolean(schema) ||
                SwaggerV2TypeChecker_1.SwaggerV2TypeChecker.isInteger(schema) ||
                SwaggerV2TypeChecker_1.SwaggerV2TypeChecker.isNumber(schema) ||
                SwaggerV2TypeChecker_1.SwaggerV2TypeChecker.isString(schema))
                ...
                else if (SwaggerV2TypeChecker_1.SwaggerV2TypeChecker.isInteger(schema) ||
                    SwaggerV2TypeChecker_1.SwaggerV2TypeChecker.isNumber(schema))
            ...
            // INSTANCE TYPE CASE
            else if (SwaggerV2TypeChecker_1.SwaggerV2TypeChecker.isArray(schema))
                union.push(Object.assign(Object.assign({}, schema), { items: SwaggerV2Upgrader.convertSchema(definitions)(schema.items), examples: schema.examples
                        ? Object.fromEntries(schema.examples.map((v, i) => [`v${i}`, v]))
                        : undefined }));
            else if (SwaggerV2TypeChecker_1.SwaggerV2TypeChecker.isObject(schema))
            ...
            else if (SwaggerV2TypeChecker_1.SwaggerV2TypeChecker.isReference(schema))
    ...
    const retrieveObject = (definitions) => (input, visited = new Set()) => {
        var _a, _b;
        if (SwaggerV2TypeChecker_1.SwaggerV2TypeChecker.isObject(input))
...
//# sourceMappingURL=SwaggerV2Upgrader.js.map