...
(function (JsonDescriptor) {
    JsonDescriptor.cascade = (props) => {
        const accessors = (props.schema.$ref.split(props.prefix)[1] ??
            props.schema.$ref.split("/").at(-1)).split(".");
        const pReferences = accessors
            ...
            .reverse()
            .filter((schema, i) => i === 0 || !!schema?.description);
    ...
    JsonDescriptor.take = (o) => {
        const result = [
            ...
            ...Object.entries(o.properties ?? {})
                .filter(([_key, value]) => OpenApiTypeChecker.isReference(value) &&
                !!value.description?.length)
                .map(([key, value]) => `### Description of {@link ${NamingConvention.variable(key) ? key : JSON.stringify(key)}} property:\n\n` +
                (value.description ?? "")
                    .split("\n")
                    .map((str) => `> ${str}`)
                    .join("\n")),
...
export { JsonDescriptor };
//# sourceMappingURL=JsonDescriptor.mjs.map
