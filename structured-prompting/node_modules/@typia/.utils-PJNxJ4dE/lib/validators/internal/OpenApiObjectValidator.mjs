...
(function (OpenApiObjectValidator) {
    OpenApiObjectValidator.validate = (ctx) => {
        ...
        return [
            ...Object.entries(ctx.schema.properties ?? {}).map(([key, value]) => OpenApiStationValidator.validate({
                ...ctx,
                schema: value,
                value: ctx.value[key],
                path: ctx.path +
            ...
            ...(typeof ctx.schema.additionalProperties === "object" &&
                ...
                ? Object.entries(ctx.value)
                    .filter(([key]) => Object.keys(ctx.schema.properties ?? {}).includes(key) ===
                    false)
                    .map(([key, value]) => OpenApiStationValidator.validate({
                    ...ctx,
