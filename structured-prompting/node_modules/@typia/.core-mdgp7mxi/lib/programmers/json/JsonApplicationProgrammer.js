...
(function (JsonApplicationProgrammer) {
    JsonApplicationProgrammer.validate = (props) => {
        var _a;
        if (props.explore.top === false)
            return JsonSchemasProgrammer_1.JsonSchemasProgrammer.validate(props);
        const output = [];
        const valid = props.metadata.size() === 1 &&
        ...
        const object = (_a = props.metadata.objects[0]) === null || _a === void 0 ? void 0 : _a.type;
        if (object !== undefined) {
    ...
    JsonApplicationProgrammer.write = (props) => {
        const app = JsonApplicationProgrammer.writeApplication({
    ...
    JsonApplicationProgrammer.writeApplication = (props) => {
        ...
        const functions = object.properties
            ...
            MetadataSchema_1.MetadataSchema.unalias(p.value).functions.length === 1)
            ...
            function: MetadataSchema_1.MetadataSchema.unalias(r.value).functions[0],
            description: r.description,
            jsDocTags: r.jsDocTags,
            collect,
        }));
        const { components, schemas } = JsonSchemasProgrammer_1.JsonSchemasProgrammer.writeSchemas({
            version: props.version,
