...
(function (LlmApplicationProgrammer) {
    LlmApplicationProgrammer.write = (props) => {
        const application = LlmApplicationProgrammer.writeApplication({
        ...
        const typeNode = props.context.importer.type({
            file: "typia",
        ...
        return ts.factory.createCallExpression(props.context.importer.internal("llmApplicationFinalize"), props.name ? [ts.factory.createTypeReferenceNode(props.name)] : undefined, [
            ts.factory.createAsExpression(ts.factory.createSatisfiesExpression(LiteralFactory.write(application), typeNode), typeNode),
    ...
    LlmApplicationProgrammer.validate = (props) => {
        ...
        if (props.explore.top === false)
            if (props.explore.object === props.top?.objects[0]?.type &&
        ...
        if (object !== undefined) {
            ...
            for (const p of object.properties) {
                const rawName = p.key.getSoleLiteral();
                ...
                if (value.functions.length) {
                    ...
                    const prefix = `LLM application's function (${name})`;
    ...
    LlmApplicationProgrammer.writeApplication = (props) => {
        ...
        const functionParameters = Object.fromEntries(metadata.objects[0].type.properties.filter((p) => p.key.isSoleLiteral() &&
            ...
            .filter((p) => p.jsDocTags.find((tag) => tag.name === "hidden" ||
            tag.name === "ignore" ||
            tag.name === "internal") === undefined)
            .map((p) => [
        ...
        const errorMessages = [];
        const application = JsonApplicationProgrammer.writeApplication({
            version: "3.1",
            metadata,
