...
(function (HttpFormDataProgrammer) {
    ...
    HttpFormDataProgrammer.validate = (props) => {
        const errors = [];
        ...
        if (props.explore.top === true) {
            ...
            const expected = props.metadata.atomics.length +
                ...
                props.metadata.constants
                    ...
                    .reduce((a, b) => a + b, 0) +
                props.metadata.natives.filter((native) => native.name === "Blob" || native.name === "File").length;
            if (atomics.size > 1)
    ...
    const decode_value = (props) => {
        const call = ts.factory.createCallExpression(props.context.importer.internal(`httpFormDataRead${NamingConvention.capitalize(props.type)}`), undefined, [props.input]);
