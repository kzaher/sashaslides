...
(function (HttpQueryProgrammer) {
    HttpQueryProgrammer.decompose = (props) => {
        ...
        const result = MetadataFactory.analyze({
            ...
            options: {
                ...
                constant: true,
                absorb: true,
                validate: (next) => HttpQueryProgrammer.validate({
    ...
    HttpQueryProgrammer.validate = (props) => {
        const errors = [];
        ...
        if (props.explore.top === true) {
            ...
            if (props.metadata.isRequired() === false) {
                if (props.allowOptional === true) {
                    const everyPropertiesAreOptional = props.metadata.size() === 1 &&
                        props.metadata.objects.length === 1 &&
                        props.metadata.objects[0].type.properties.every((p) => p.value.isRequired() === false);
    ...
    const decode_regular_property = (props) => {
        ...
        return ts.factory.createPropertyAssignment(NamingConvention.variable(key)
            ? key
            : ts.factory.createStringLiteral(key), isArray
            ? decode_array({
                context: props.context,
                metadata: value,
    ...
    const decode_value = (props) => {
        const call = ts.factory.createCallExpression(props.context.importer.internal(`httpQueryRead${NamingConvention.capitalize(props.type)}`), undefined, [props.input]);
