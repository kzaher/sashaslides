    ...
    const validate = () => {
        ...
        return (props) => {
            ...
            if (props.explore.top === true) {
                const onlyObject = props.metadata.size() === 1 &&
                    props.metadata.objects.length === 1 &&
                    props.metadata.objects[0].type.properties.every((p) => p.key.isSoleLiteral()) &&
            ...
            if (props.metadata.atomics.length) {
                const numbers = ProtobufUtil_1.ProtobufUtil.getNumbers(props.metadata);
                const bigints = ProtobufUtil_1.ProtobufUtil.getBigints(props.metadata);
            ...
            if (props.metadata.arrays.length &&
                props.metadata.arrays.some((array) => array.type.value.isRequired() === false ||
                    array.type.value.nullable === true))
                noSupport("optional type in array");
            // UNION IN ARRAY
            if (props.metadata.arrays.length &&
                props.metadata.arrays.some((a) => {
                    var _a;
            ...
            if (props.metadata.arrays.length &&
                props.metadata.arrays.some((a) => a.type.value.maps.length ||
            ...
            if (props.metadata.objects.length &&
                props.metadata.objects.some((obj) => obj.type.properties.filter((p) => !p.key.isSoleLiteral()).length >
                    1))
            ...
            if (props.metadata.objects.length &&
                props.metadata.objects.some((obj) => obj.type.properties.some((p) => p.key.isSoleLiteral()) &&
            ...
            if (props.metadata.objects.length &&
                ...
                props.metadata.objects[0].type.properties.some((p) => !!p.value.arrays.length))
            ...
            if (props.metadata.objects.length &&
                props.metadata.objects.some((obj) => isDynamicObject(obj.type) &&
            ...
            if (props.metadata.maps.length &&
                props.metadata.maps.some((m) => ProtobufUtil_1.ProtobufUtil.isUnion(m.key)))
                noSupport("union key typed map");
            // KEY TYPE IS NOT ATOMIC
            if (props.metadata.maps.length &&
                props.metadata.maps.some((m) => ProtobufUtil_1.ProtobufUtil.getAtomics(m.key).size !== 1))
                noSupport("non-atomic key typed map");
            // MAP TYPE, BUT PROPERTY KEY TYPE IS OPTIONAL
            if (props.metadata.maps.length &&
                props.metadata.maps.some((m) => m.key.isRequired() === false || m.key.nullable))
            ...
            // UNION IN MAP
            if (props.metadata.maps.length &&
                props.metadata.maps.some((m) => ProtobufUtil_1.ProtobufUtil.isUnion(m.value)))
