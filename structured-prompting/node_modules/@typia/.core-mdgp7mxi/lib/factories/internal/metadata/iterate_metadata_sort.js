...
const iterate = (props) => {
    ...
    if (props.metadata.arrays.length > 1)
        props.metadata.arrays.sort((x, y) => MetadataSchema_1.MetadataSchema.covers(x.type.value, y.type.value)
            ? -1
            : MetadataSchema_1.MetadataSchema.covers(y.type.value, x.type.value)
                ? 1
                : 0);
    if (props.metadata.tuples.length > 1)
        props.metadata.tuples.sort((x, y) => {
            const xt = MetadataSchema_1.MetadataSchema.initialize();
            const yt = MetadataSchema_1.MetadataSchema.initialize();
            xt.tuples.push(x);
            yt.tuples.push(y);
            return MetadataSchema_1.MetadataSchema.covers(xt, yt)
                ? -1
                : MetadataSchema_1.MetadataSchema.covers(yt, xt)
                    ? 1
                    : 0;
        });
