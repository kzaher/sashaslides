...
const TransformerError_1 = require("../../context/TransformerError");
...
const MetadataCollection_1 = require("../../schemas/metadata/MetadataCollection");
...
(function (HttpHeadersProgrammer) {
    ...
    HttpHeadersProgrammer.validate = (props) => {
        const errors = [];
        ...
        if (props.explore.top === true) {
        ...
        else if (props.explore.nested !== null &&
            props.explore.nested instanceof MetadataArrayType_1.MetadataArrayType) {
            //----
            // ARRAY
            //----
            const atomics = HttpMetadataUtil_1.HttpMetadataUtil.atomics(props.metadata);
            const expected = props.metadata.atomics.length +
            ...
            if (HttpMetadataUtil_1.HttpMetadataUtil.isUnion(props.metadata))
            ...
            // SPECIAL KEY NAMES
            //----
            const isArray = props.metadata.arrays.length >= 1 || props.metadata.tuples.length >= 1;
            // SET-COOKIE MUST BE ARRAY
            if (typeof props.explore.property === "string" &&
            ...
            for (const prop of props.explore.object.properties) {
                ...
                if (key === null)
                    continue;
                utils_1.MapUtil.take(counter, key.toLowerCase(), () => new Set()).add(key);
            }
    ...
    const decode_regular_property = (props) => {
        ...
        return typescript_1.default.factory.createPropertyAssignment(utils_1.NamingConvention.variable(key)
            ? key
    ...
    const decode_value = (props) => props.type === "string"
        ? props.input
        : typescript_1.default.factory.createCallExpression(props.context.importer.internal(`httpHeaderRead${utils_1.NamingConvention.capitalize(props.type)}`), undefined, [props.input]);
    const decode_array = (props) => {
        const reader = props.type === "string"
            ...
            : props.context.importer.internal(`httpHeaderRead${utils_1.NamingConvention.capitalize(props.type)}`);
