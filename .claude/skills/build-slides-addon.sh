...
# dist/renderer/html2slides/addon/. Run after any change to addon-main.ts,
# convert-core.ts, extract-dom.ts, or build-addon.ts.
#
# Deploy: see renderer/html2slides/addon/README.md (manual paste, or
#   cd dist/renderer/html2slides/addon && clasp create --type slides --parentId <PRES_ID> && clasp push
set -euo pipefail
cd "$(dirname "$0")/../.."

DIST="$PWD/dist/renderer/html2slides/addon"
